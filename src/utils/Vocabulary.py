...
class Vocabulary:
	...
	def __init__(self,data_range=1.0,max_len=10,min_len=1,min_number=2):
		...
		self.df= LoadLines.read_csv() # In order to get prepeard input-target sntences, read .csv file or run LoadLines().get_input_target()
		self.data_range= 2 * round((data_range*self.df.input_val.size)/2) # Round always to number divisible by 2
		print(self.df.input_val.size)
		self.list_of_words=[]
		self.word_number=2 
	...
	def get_model_data(self):
		...
		decoder_output = np.zeros((len(self.Input_set),target_max_words,len(target_index_word)),dtype='float32')
		print('target',target_max_words)
		print('input',input_max_words)
		for line,(input,target) in enumerate(zip(self.Input_set,self.Target_set)):
			...
			for timestemp,token in enumerate(target):
				decoder_input[line,timestemp,target_word_index[token]]=1.
				if timestemp>0:
					decoder_output[line,timestemp-1,target_word_index[token]]=1.
	...
	def vocabulary_parameters(self):
		...
		'''
		print("-----------------------------------")
		print(f'Input and Target sentences: {self.df.input_val.size}')
		print(f'Data range: {self.data_range} / {self.df.input_val.size}')
	...
	def cleaning(self):
		...
		input,target = [],[]
		df = self.df # faster iterating
		filtered_index = 0 # Number of filltered sentences pairs.
		for row in df.itertuples():
			
			input_line=row.input_val
			target_line=row.target_val
			...
			try:
				...
				if self.filter_length(input_line,target_line):
					...
					filtered_index+=1
					print(f'{filtered_index}/{self.data_range}')
	...
	def filter_length(self, input_line, target_line):
		...
		args:
			...
			return: bool
		'''
		if len(target_line.split(' '))<=self.MAX_LEN and len(input_line.split(' '))<=self.MAX_LEN:
			return True
...
if __name__ == '__main__':
	vocab=Vocabulary(data_range=0.2,
							max_len=11,
							min_len=1,
							min_number=2) # initiate vocabulary object with specific parameteres
