...
class ChatBot():
    def __init__(self) -> None:
        ...
        self.model=tf.keras.models.load_model('models/LSTM_model_2.h5')
        
        self.input_tokens= len(self.input_vocab)
        self.target_tokens=len(self.target_vocab)
        print(self.input_tokens, self.target_tokens)
    ...
    def step_by_step_model(self):
        ...
        decoder_inputs = Input(shape=(None, self.target_tokens))
        decoder_lstm = LSTM(256, return_sequences=True, return_state=True)
        decoder_dense = Dense(self.target_tokens, activation='softmax')
    ...
    def sentence_to_matrix(self,sentence)->np.array:
        ...
        user_input_matrix = np.zeros((1, 12, self.input_tokens),dtype='float32')
    ...
    def generate_response(self, user_input)->str:
        ...
        chatbot_response = 'Chatbot: '+chatbot_response.replace("<BOS>",'').replace("<EOS>",'')
    ...
    def get_decoded_sentence(self,expression):
        ...
        target_seq = np.zeros((1, 1,  self.target_tokens))
        target_seq[0, 0, 1] = 1.
        decoded_sentence = ''
...
if __name__ == '__main__':
    bot = ChatBot()
    x=bot.generate_response("jango jango jango")
    print(x)